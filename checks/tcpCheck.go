...
func (s *StatusCodeTcp) checkPort() {
	...
	if len(c.HostCheck) == 0 {
		utils.LogWarn("checkItems: No server for check list")
	}
	port := c.PortCheck
	...
	for _, server := range c.HostCheck {
		utils.LogDebug("checkItems: %s", server)
		conn, err := net.DialTimeout("tcp", server+":"+strconv.Itoa(port), 1*time.Second)
		if err != nil {
			utils.LogWarn("Connection failed: %s", server)
		} else {
			newPool = append(newPool, server)
			newSet[server] = struct{}{}
			utils.LogDebug("Connection established: %s", server)
			connErr := conn.Close()
			if connErr != nil {
				utils.LogError("Error close connections %s", connErr)
			}
		}
	}
	poolMutex.Lock()
	...
	poolMutex.Unlock()
	metrics.PoolSize.Set(float64(len(newPool)))
	utils.LogInfo("ValidPoolHost %s", newPool)
}
